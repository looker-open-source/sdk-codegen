...
func TestAuthSession_Do_Authorization(t *testing.T) {
	...
	t.Run("Do() sets Authorization header with access token fetched from /login", func(t *testing.T) {
		...
		err := s.Do(nil, "GET", apiVersion, path, nil, nil, nil)

		if err != nil {
			t.Errorf("s.Do() call failed: %v", err)
		}
	})
...
// 		if err != nil {
// 			t.Errorf("First s.Do() call failed: %v", err)
// 		}
...
// 		if err != nil {
// 			t.Errorf("Second s.Do() call failed: %v", err)
// 		}
