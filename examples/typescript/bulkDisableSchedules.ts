...
const bulkDisableSchedules = async (userID?: number): Promise<boolean | Error> => {
    ...
    try {
        ...
        console.log(`Successfully disabled all schedules ${userID ? `for user ${userID}.` : 'for all users.'}`)
        return true
    } catch (e) {
