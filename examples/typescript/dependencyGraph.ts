...
const getPDTGraph = async (modelname: string, filename: string) => {
   if(modelname) {
      const res = await sdk.ok(sdk.graph_derived_tables_for_model({model:modelname}))
      ...
      throw Error(`Model name not specified, please specifiy a model name to pull pdt dependency graph for.`)
    }
}
