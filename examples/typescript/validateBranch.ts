...
const validateBranch = async (projectName: string, branch: string): Promise<IProjectError[]> => {
    ...
    if (workspace.workspace_id == 'production') {
        const updatedWorkspace = await sdk.ok(sdk.update_session({workspace_id: 'dev'}))
    ...
    if(currentBranch.name == branch) {
        try {
            ...
            if(validateResults.errors.length > 0) {
                console.log(`There are errors with this lookml project. Errors: ${JSON.stringify(validateResults.errors)}`)
                return validateResults.errors
            } else {
                console.log('There are no errors with this lookml project.')
            }
        } catch (e) {
            throw new Error(`There was an error validating this project. Here is the full message: ${e}.`)
        }
    } else {
        try {
            await sdk.ok(sdk.update_git_branch(projectName, {
                name: branch
            }))
        } catch(e) {
            throw new Error(`There was an error checking out ${branch}. Check to make sure the branch inputted is a valid branch in the repo. Error: ${e}.`)
        }
