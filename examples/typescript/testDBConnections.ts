...
import {IDBConnection, IDBConnectionTestResult} from '@looker/sdk'
...
const runConnectionTests = async (): Promise<ConnectionTestObj[]> => {
    ...
    const tests: ConnectionTestObj[] = await Promise.all(connections.slice(0,10).map(async (c) => {

        try {
            const connection = await sdk.ok(sdk.test_connection(c.name, new DelimArray<string>(c.dialect.connection_tests)))
            ...
            // throw new Error(`There was an error running connection test for ${c.name}. Full error: ${e}`)
            ...
            console.error(`There was an error running connection test for ${c.name}. Full error: ${e}`)
        }
    }))
